...
class PortfolioOptEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        asset1mean = np.array([1.25, 2, 4, 5, 3, 2, 3, 6, 9, 7]).reshape(
            1, -1
        )  # Up and down all the way
        asset2mean = np.array([5, 3, 2, 2, 1.25, 4, 5, 6, 7, 8]).reshape(
            1, -1
        )  # Down intially then up
        asset3mean = np.array([3, 5, 6, 9, 10, 8, 4, 2, 1.25, 4]).reshape(
            1, -1
        )  # Up initially then down
        ...
        self.observation_space = spaces.Box(
            -20000, 20000, shape=(self.obs_length,), dtype=np.float32
        )
    ...
    def _generate_asset_prices(self):
        asset_prices = np.array(
            [
                self.np_random.normal(mu, sig)
                for mu, sig in zip(
                    self.asset_price_means.flatten(), self.asset_price_var.flatten()
                )
            ]
        ...
        for idx, a in enumerate(action):
            ...
            # Buy a shares of asset
                ...
                if self.cash < purchase_cost:
                    a = np.floor(
                        self.cash / (asset_prices[idx] * (1 + self.buy_cost[idx]))
                    )
                    purchase_cost = asset_prices[idx] * a * (1 + self.buy_cost[idx])
                self.holdings[idx] += a
    ...
    def _update_state(self):
        self.state = np.hstack(
            [self.cash, self.asset_prices[:, self.step_count], self.holdings],
            dtype=np.float32,
        )
