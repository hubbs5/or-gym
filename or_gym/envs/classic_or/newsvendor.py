...
class NewsvendorEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.action_space = spaces.Box(
            low=np.zeros(1), high=np.array([self.max_order_quantity]), dtype=np.float32
        )
    ...
    def _STEP(self, action):
        ...
        order_qty = max(
            0,  # Ensure order > 0
            min(action, self.max_inventory - self.state[5:].sum()),
        )  # Cap inventory
        ...
        purchase_cost = (
            excess_inventory * self.cost * order_qty * self.gamma**self.lead_time
        )
